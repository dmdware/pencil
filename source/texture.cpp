...
LoadedTex *LoadBMP(const char *fullpath)
{
	...
	*/
	...
	/*
	int32_t stride = pImage->channels * pBitbldg->sizex;
	int32_t i;
	int32_t y2;
	int32_t temp;

	for(int32_t y = 0; y < pImage->sizey/2; y++)
	{
		y2 = pImage->sizey - y;
...
LoadedTex *LoadTGA(const char *fullpath)
{
	...
	int32_t channels = 0;					// The channels of the image (3 = RGA : 4 = RGBA)
	int32_t stride = 0;						// The stride (channels * width)
	...
	if(imageType != TGA_RLE)
	{
		...
		if(bits == 24 || bits == 32)
		{
			...
			for(int32_t y = 0; y < height; y++)
			{
				...
				{
					int32_t temp     = pLine[i];
					pLine[i]     = pLine[i + 2];
					pLine[i + 2] = temp;
				}
			}
		}
		// Check if the image is a 16 bit image (RGB stored in 1 uint16_t)
		else if(bits == 16)
		{
			uint16_t pixels = 0;
			int32_t r=0, g=0, b=0;
...
void DecodeJPG(jpeg_decompress_struct* cinfo, LoadedTex *pImageData)
{
	...
	int32_t rowSpan = cinfo->image_width * cinfo->num_components;
	...
	for (int32_t i = 0; i < pImageData->sizey; i++)
...
LoadedTex *LoadPNG(const char *fullpath)
{
	...
	Log("PNG header "<<relative<<" "
		<<(int32_t)header[0]<<","<<(int32_t)header[1]<<","<<(int32_t)header[2]<<","<<(int32_t)header[3]<<","
		<<(int32_t)header[4]<<","<<(int32_t)header[5]<<","<<(int32_t)header[6]<<","<<(int32_t)header[7]);
	*/
	...
	case PNG_COLOR_TYPE_GRAY:
		...
		break;
	default:
		Log("png %s color type %d not supported\r\n", fullpath, (int32_t)png_get_color_type(png_ptr, info_ptr));
...
bool FindTexture(uint32_t &textureidx, const char* relative)
{
	char corrected[1024];
...
bool TextureLoaded(uint32_t texture, const char* relative, bool transp, bool clamp, bool mipmaps, uint32_t& texindex, bool reload)
{
	char corrected[1024];
	strcpy(corrected, relative);
	CorrectSlashes(corrected);
...
bool CreateTex(LoadedTex* pImage, unsigned int* texname, bool clamp, bool mipmaps)
{
	...
	//Log("mipmaps:"<<(int32_t)mipmaps<<" :"<<relative);
	...
	int32_t kbleft = GetFreeVideoMem();
	Log("VRAM left: %d kB", kbleft);

	return true;
}

bool CreateTex(uint32_t &texindex, const char* relative, bool clamp, bool mipmaps, bool reload)
{
	CHECKGLERROR();
	...
	if(!CreateTex(pImage, &texname, clamp, mipmaps))
	{
		...
		char m[128];
		sprintf(m, "Error creating texture %s", relative);
		ErrMess("Error", m);
		return false;
	}
...
void Blit(LoadedTex* src, LoadedTex* dest, Vec2i pos)
{
	...
	for(int32_t x=0; x<src->sizex; x++)
	{
		...
		for(int32_t y=0; y<src->sizey; y++)
		{
			...
			int32_t srcpixel = x*src->channels + y*src->channels*src->sizex;
			int32_t destpixel = (x+pos.x)*dest->channels + (y+pos.y)*dest->channels*dest->sizex;
...
void SaveJPEG(const char* fullpath, LoadedTex* image, float quality)
{
	...
	jpeg_set_quality (&cinfo, 100*quality, (boolean)TRUE);
	jpeg_start_compress(&cinfo, (boolean)TRUE);

	JSAMPROW row_pointer;
	int32_t row_stride = image->sizex * 3;
...
void FlipImage(LoadedTex* image)
{
	...
	int32_t y2;
	byte temp[4];
	int32_t stride = image->sizex * image->channels;

	for(int32_t y=0; y<image->sizey/2; y++)
	{
		y2 = image->sizey - y - 1;
...
int32_t SaveBMP(const char* fullpath, LoadedTex* image)
{
	...
	bitmapInfoHeader.biSizeImage = image->sizex * iabs(image->sizey) * 3;
...
#if 0
	for(int32_t imageIdx = image->channels * (image->sizex - 1);
...
void StreamRaw(FILE* fp, uint32_t* texname, Vec2i fullsz, Vec2i srcpos, Vec2i srcsz, Vec2i destsz)
{
	if(srcpos.x < 0)
		srcpos.x = 0;
	if(srcpos.y < 0)
	...
	int32_t stridex = srcsz.x / destsz.x * 3;
	int32_t stridey = srcsz.y / destsz.y * 3 * fullsz.x;
	...
	for(int32_t y=srcpos.y; srcpos.y < srcpos.y+srcsz.y; y+=stridey)
		for(int32_t x=srcpos.x; srcpos.x < srcpos.x+srcsz.x; x+=stridex)
		{
...
void Extract(LoadedTex* original, LoadedTex* empty, int32_t x1, int32_t y1, int32_t x2, int32_t y2)
{
	...
	for(int32_t x=x1; x<x2; x++)
		...
		{
			int32_t i1 = (x + y * original->sizex) * original->channels;
			int32_t i2 = ((x-x1) + (y-y1) * empty->sizex) * empty->channels;
			
			for(int32_t c=0; c<original->channels; c++)
...
void Resample(LoadedTex* original, LoadedTex* empty, Vec2i newdim)
{
	...
	for(int32_t cy = 0; cy < newdim.y; cy++)
	{
		for(int32_t cx = 0; cx < newdim.x; cx++)
		{
			int32_t pixel = cy * (newdim.x * original->channels) + cx*original->channels;
			int32_t nearestMatch =  (int32_t)(cy / scaleH) * original->sizex * original->channels + (int32_t)(cx / scaleW) * original->channels;
